function region = findCWVortex(M,x,y,c, treshold)
min_val = min(M, [], 'all');
levels  = linspace(min_val, -2/pi, 20);
...
for i=length(level):-1:1
    nb_vertices_i = sum(nb_vertices(1:(i-1)))+i+1; % Index of the first vertex at level i
...
region = centeredRegions_new(1);
    ...
    centeredRegions_new = min_point;
    region = centeredRegions_new(1);
    'Warning : min_point was taken as a region'
end
